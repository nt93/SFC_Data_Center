# This code is to be run on Service Function 2
...
while True:
    ...
    if eth_addr(packet[0:6]) == "fa:16:3e:00:36:a6":
	    ...
	    if eth_protocol == 8 :
        	...
        	print 'Version : ' + str(version) + ' IP Header Length : ' + str(ihl) + ' TTL : ' + str(ttl) + ' Protocol : ' + str(protocol) + ' Source Address : ' + str(s_addr) + ' Destination Address : ' + str(d_addr)
            	...
            	if protocol == 17 :
            		...
            		data = packet[h_size:]
			data_str = str(data)
			print str(packet).find("SIGSFC")
			if data_str.find("SIGSFC") != -1:
				...
				img_file=open('image_client','wb')
    				...
    				# img_modi=Image.open('image_client').convert('L')                #Converting to Gray Scale
                                img_modi=Image.open('image_client')
                                img_modi.save("Server_img_compressed.jpg",optimize=True,quality=25)
				...
				ip_header = makeip_header(s_addr,d_addr)
				user_data = ""
    				img_modifile=open('Server_img_compressed.jpg','rb')
